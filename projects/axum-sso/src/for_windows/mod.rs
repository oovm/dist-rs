...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mount_point = U16CString::from_str(matches.mount_point).unwrap();
    let mut flags = MountFlags::ALT_STREAM | MountFlags::ALLOW_IPC_BATCHING;
    if matches.dokan_debug {
        flags |= MountFlags::DEBUG | MountFlags::STDERR;
    }
    if matches.removable {
        flags |= MountFlags::REMOVABLE;
    }
    init();
    Drive::new()
        .mount_point(&mount_point)
        .single_thread(matches.single_thread)
        .flags(flags)
        .mount(&MemFsHandler::new()).unwrap();
